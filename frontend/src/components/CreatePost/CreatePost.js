...
const CreatePost = ({ closeModal }) => {
  ...
  const handleShare = () => {
    console.log("Shared Post", { image: selectedImage, caption, hashtag });
    closeModal();
  };
        ...
        <div className="mb-4">
          {selectedImage ? (
            <img
              src={selectedImage}
              alt="Selected"
              className="w-full h-48 sm:h-64 bg-gray-200 flex items-center justify-center mb-4 rounded-md object-cover"
            />
          ) : (
          ...
          <textarea
            ...
            className="w-full p-2 border border-gray-300 mb-4 rounded h-auto"
            cols="30"
            value={caption}
          ...
          />
          <button className="w-full bg-blue-500 text-white rounded p-2 hover:bg-blue-600" onClick={handleShare}>
            Share
          </button>
        </div>
      </div>
